...
const adminPin = '6969'
const storageKey = 'happy-kokoro-inventario'
const recipesStorageKey = 'happy-kokoro-recetas'
const gruposStorageKey = 'happy-kokoro-grupos'

const gruposIniciales = ['Hamburguesas', 'Postres', 'Otros']

const ingredientesBase = [
  { id: 1, nombre: 'Harina de avena', grupo: 'Otros', cantidad: 4, unidad: 'kg', precioCompra: 3200, stockMinimo: 2, imagenUrl: '' },
  { id: 2, nombre: 'Mantequilla de mani', grupo: 'Otros', cantidad: 8, unidad: 'frascos', precioCompra: 2500, stockMinimo: 3, imagenUrl: '' },
  { id: 3, nombre: 'Chocolate bitter', grupo: 'Otros', cantidad: 12, unidad: 'barras', precioCompra: 1800, stockMinimo: 4, imagenUrl: '' },
]

const recetasIniciales = [
  {
    id: 1,
    nombre: 'Kokoro bites clasicos',
    rendimiento: '12 unidades',
    imagenUrl: '',
    ingredientes: ['Harina de avena', 'Mantequilla de mani', 'Chocolate bitter'],
    pasos: 'Mezclar la base, formar bolitas, cubrir con chocolate y refrigerar.',
  },
  {
    id: 2,
    nombre: 'Barritas happy',
    rendimiento: '8 barritas',
    imagenUrl: '',
    ingredientes: ['Harina de avena', 'Mantequilla de mani'],
    pasos: 'Compactar la mezcla en molde, enfriar y cortar en porciones.',
  },
]
...
function App() {
  ...
  const [ingredientes, setIngredientes] = useState(() => {
    ...
    return guardado ? JSON.parse(guardado) : ingredientesBase
  })
  const [recetas, setRecetas] = useState(() => {
    const guardado = localStorage.getItem(recipesStorageKey)
    return guardado ? JSON.parse(guardado) : recetasIniciales
  })
  const [grupos, setGrupos] = useState(() => {
    const guardado = localStorage.getItem(gruposStorageKey)
    return guardado ? JSON.parse(guardado) : gruposIniciales
  })
  ...
  useEffect(() => {
    localStorage.setItem(storageKey, JSON.stringify(ingredientes))
  }, [ingredientes])

  useEffect(() => {
    localStorage.setItem(recipesStorageKey, JSON.stringify(recetas))
  }, [recetas])

  useEffect(() => {
    localStorage.setItem(gruposStorageKey, JSON.stringify(grupos))
  }, [grupos])
  ...
  const guardarIngrediente = () => {
    ...
    if (editando) {
      setIngredientes((actuales) =>
        actuales.map((item) => (item.id === editando ? { ...item, ...ingredienteGuardado } : item)),
      )
    } else {
      setIngredientes((actuales) => [
        ...actuales,
        { id: Date.now(), ...ingredienteGuardado },
      ])
    }
  ...
  const cambiarCantidad = (id, nuevaCantidad) => {
    setIngredientes((actuales) =>
      actuales.map((item) =>
        item.id === id ? { ...item, cantidad: Math.max(0, nuevaCantidad) } : item,
      ),
    )
  }

  const eliminarIngrediente = (id) => {
    setIngredientes((actuales) => actuales.filter((item) => item.id !== id))
  }

  const agregarGrupo = () => {
    const nombre = nuevoGrupo.trim()
    if (nombre === '' || grupos.includes(nombre)) return
    setGrupos((actuales) => [...actuales, nombre])
    setNuevoGrupo('')
  }

  const eliminarGrupo = (nombre) => {
    if (grupos.length <= 1) return
    const grupoDestino = grupos.find((g) => g !== nombre) || 'Otros'
    setIngredientes((actuales) =>
      actuales.map((item) =>
        item.grupo === nombre ? { ...item, grupo: grupoDestino } : item
      )
    )
    setGrupos((actuales) => actuales.filter((g) => g !== nombre))
  }
  ...
  const guardarReceta = () => {
    ...
    const recetaGuardada = {
      ...
      ingredientes: formularioReceta.ingredientes
        .split(',')
    ...
    if (recetaEditando) {
      setRecetas((actuales) =>
        actuales.map((receta) => (receta.id === recetaEditando ? { ...receta, ...recetaGuardada } : receta)),
      )
    } else {
      setRecetas((actuales) => [...actuales, { id: Date.now(), ...recetaGuardada }])
    }
  ...
  const eliminarReceta = (id) => {
    setRecetas((actuales) => actuales.filter((receta) => receta.id !== id))
  }
  ...
  const buscarImagenIngrediente = (nombre) => {
    const encontrado = ingredientes.find(
        ...
        <section className="page-screen">
          ...
          {mostrarFormulario && (
            <div className="ingredient-form">
              ...
              />
              <button className="primary-button compact" onClick={guardarIngrediente}>
          ...
          {grupos.map((grupo) => {
            ...
            return (
              <div key={grupo}>
                <h3 style={{ color: 'white', margin: '24px 0 10px 0', borderBottom: '1px solid #444', paddingBottom: '6px' }}>
                  {grupo}
                </h3>
                      ...
                      <article className="ingredient-card" key={item.id}>
                        ...
                        {item.imagenUrl && (
                          <img
                            src={item.imagenUrl}
                            alt={item.nombre}
                            style={{ width: '100%', borderRadius: '6px', marginBottom: '6px', objectFit: 'cover', maxHeight: '80px' }}
                          />
                        )}
                        <div>
                        ...
                        <div>
                          <p className="stock-line" style={{ color: colorStock }}>
        ...
        <section className="page-screen">
          ...
          <div className="inventory-toolbar">
            <button
              ...
              onClick={() => {
                setModoEdicionRecetas(!modoEdicionRecetas)
                setMostrarFormularioReceta(false)
                limpiarFormularioReceta()
              }}
            >
            ...
            {modoEdicionRecetas && (
              <button className="primary-button compact" onClick={abrirNuevaReceta}>Agregar receta</button>
            )}
          </div>
          ...
          {modoEdicionRecetas && mostrarFormularioReceta && (
            <div className="recipe-form">
              <input
                value={formularioReceta.nombre}
                onChange={(e) => setFormularioReceta({ ...formularioReceta, nombre: e.target.value })}
                placeholder="Nombre de la receta"
              />
              <input
                value={formularioReceta.rendimiento}
                onChange={(e) => setFormularioReceta({ ...formularioReceta, rendimiento: e.target.value })}
                placeholder="Rinde, ejemplo: 12 unidades"
              />
              <input
                value={formularioReceta.imagenUrl}
                onChange={(e) => setFormularioReceta({ ...formularioReceta, imagenUrl: e.target.value })}
                placeholder="URL imagen opcional"
              />
              <input
                value={formularioReceta.ingredientes}
                onChange={(e) => setFormularioReceta({ ...formularioReceta, ingredientes: e.target.value })}
                placeholder="Ingredientes separados por coma"
              />
              <textarea
                value={formularioReceta.pasos}
                onChange={(e) => setFormularioReceta({ ...formularioReceta, pasos: e.target.value })}
                placeholder="Pasos de preparacion"
              />
              <div className="form-actions">
                <button className="primary-button compact" onClick={guardarReceta}>
              ...
              >
                {modoEdicionRecetas && (
                  <button
                    ...
                    onClick={(e) => { e.stopPropagation(); abrirEditorReceta(receta) }}
                  >⚙</button>
                )}
                {receta.imagenUrl && (
                  <img className="recipe-image" src={receta.imagenUrl} alt={receta.nombre} />
                )}
                <h3>{receta.nombre}</h3>
                ...
                {modoEdicionRecetas && (
                  <button
                    ...
                    onClick={(e) => { e.stopPropagation(); eliminarReceta(receta.id) }}
                  >Eliminar</button>
                )}
              </article>
            ))}
      ...
      {paginaActual === 'recetas' && recetaAbierta && (
        <section className="page-screen">
          ...
          {recetaAbierta.imagenUrl && (
            <img
              src={recetaAbierta.imagenUrl}
              alt={recetaAbierta.nombre}
          ...
          {recetaAbierta.rendimiento && (
            <p style={{ color: '#aaa', marginBottom: '20px', fontSize: '0.95rem' }}>
              Rinde: {recetaAbierta.rendimiento}
            </p>
          )}

          <div>
            <button style={estiloSeccionBtn('ingredientes')} onClick={() => toggleSeccion('ingredientes')}>
              <span>Ingredientes</span>
                      ...
                      {imagenTooltip === ing && imagenIng && (
                        <div style={{
                          ...
                          <img
                            src={imagenIng}
                            alt={ing}
                            style={{ width: '140px', height: '100px', objectFit: 'cover', borderRadius: '6px', display: 'block' }}
                          />
          ...
          <div style={{ marginTop: '8px' }}>
            <button style={estiloSeccionBtn('preparacion')} onClick={() => toggleSeccion('preparacion')}>
              <span>Preparación</span>
              <span>{seccionAbierta === 'preparacion' ? '▲' : '▼'}</span>
            </button>
            {seccionAbierta === 'preparacion' && (
              <p style={{ color: '#ddd', lineHeight: '1.7', whiteSpace: 'pre-wrap', padding: '12px 0' }}>
                {recetaAbierta.pasos}
              </p>
            )}
          </div>
        ...
        <section className="page-screen admin-screen">
          ...
          {!adminAutorizado ? (
            ...
            <div className="admin-grid">
              ...
              <div className="summary-card">
                <span>Productos elaborados vendidos</span>
                <strong>0</strong>
              </div>
            </div>
          )}
